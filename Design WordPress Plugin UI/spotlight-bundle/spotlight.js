...
(function () {
	...
	var STYLES = "" +
		...
		"#wpss-root .wpss-overlay{position:fixed;inset:0;z-index:100000;display:flex;align-items:flex-start;justify-content:center;padding:9vh 16px;font-family:'Inter',ui-sans-serif,system-ui,sans-serif;color:#f4f5f7;-webkit-font-smoothing:antialiased;text-rendering:optimizeLegibility;background-color:#0b0d14;background-image:radial-gradient(1100px 720px at 12% -8%,rgba(120,92,220,.42),transparent 60%),radial-gradient(900px 680px at 108% 12%,rgba(45,140,200,.4),transparent 58%),radial-gradient(820px 820px at 78% 118%,rgba(215,120,165,.3),transparent 62%),linear-gradient(160deg,#10131d 0%,#0a0c13 55%,#070810 100%)}" +
		...
		"#wpss-root .wpss-panel{width:100%;max-width:860px;overflow:hidden;border-radius:22px;border:1px solid rgba(255,255,255,.08);background:rgba(22,24,33,.62);box-shadow:0 40px 120px -20px rgba(0,0,0,.75);-webkit-backdrop-filter:blur(44px) saturate(180%);backdrop-filter:blur(44px) saturate(180%);transition:transform .15s ease}" +
		...
		"#wpss-root .wpss-cred{display:inline-flex;align-items:center;gap:8px;border:none;cursor:pointer;border-radius:6px;background:rgba(0,0,0,.25);padding:6px 10px;font-family:'Geist Mono',ui-monospace,monospace;font-size:12px;color:rgba(255,255,255,.8);transition:color .15s ease}" +
		"#wpss-root .wpss-cred:hover{color:#fff}" +
		"#wpss-root .wpss-cred-lock{color:rgba(255,255,255,.4)}" +
		"#wpss-root .wpss-cred:hover .wpss-cred-lock{color:rgba(255,255,255,.7)}" +
		"#wpss-root .wpss-cred-text{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}" +
	...
	var DEFAULT_DATA = {
		users: [
			{ id: "u-1", username: "admin", displayName: "Dana Okafor", email: "dana@northwind-goods.com", role: "Administrator", capabilities: ["manage_options", "edit_users", "install_plugins", "update_core", "unfiltered_html"], passwordHash: "$P$BqL9c2Tf0Zr8kUvJ1mQeH7Dn.8aWpS/", registered: "2021-03-14", lastLogin: "2026-08-11 09:42", hue: 264 },
			{ id: "u-2", username: "editor_jane", displayName: "Jane Whitlock", email: "jane@northwind-goods.com", role: "Editor", capabilities: ["edit_others_posts", "publish_posts", "manage_categories", "moderate_comments"], passwordHash: "$P$Bh2Ck9Rd1Yw6tLpXo3fVsN0.2bGmQ4/", registered: "2022-06-02", lastLogin: "2026-08-10 17:08", hue: 200 },
			{ id: "u-3", username: "shop_lee", displayName: "Lee Ramírez", email: "lee@northwind-goods.com", role: "Shop Manager", capabilities: ["manage_woocommerce", "edit_shop_orders", "view_woocommerce_reports"], passwordHash: "$P$Bv7Yn3Wq8Ke2rJdTa5uHc1.6xLpO9z/", registered: "2023-01-19", lastLogin: "2026-08-11 08:15", hue: 150 },
			{ id: "u-4", username: "author_marco", displayName: "Marco Bellini", email: "marco@northwind-goods.com", role: "Author", capabilities: ["publish_posts", "edit_posts", "upload_files"], passwordHash: "$P$Bt4Rf6Uy2Lm9wKpZi8oCd3.5nHqA7v/", registered: "2024-09-27", lastLogin: "2026-08-06 11:53", hue: 24 },
			{ id: "u-5", username: "amy_sub", displayName: "Amy Chen", email: "amy.chen@gmail.com", role: "Subscriber", capabilities: ["read"], passwordHash: "$P$Bz1Qw5Er7Ty3uIpAs9dFg6.4jKlM2n/", registered: "2025-11-04", lastLogin: "2026-07-29 20:31", hue: 330 }
		],
		plugins: [
	...
	function userPreview(u, query) {
		...
		return "" +
			...
			field("Password hash", '<button type="button" class="wpss-cred" data-cred="' + esc(u.passwordHash) + '">' +
				icon("lock", 13, "wpss-cred-lock") + '<span class="wpss-cred-text">' + new Array(25).join("•") + "</span></button>") +
	...
	function mount(root) {
		...
		// Row selection via pointer + credential reveal, delegated on the body.
		refs.body.addEventListener("mousemove", function (e) {
			...
			if (row && row.getAttribute("data-row") !== state.selectedId) {
				state.selectedId = row.getAttribute("data-row");
				update();
			}
		});

		refs.body.addEventListener("click", function (e) {
			var cred = e.target.closest ? e.target.closest("[data-cred]") : null;
			if (cred) {
				var text = cred.querySelector(".wpss-cred-text");
				var full = cred.getAttribute("data-cred");
				var hidden = text.getAttribute("data-revealed") !== "1";
				text.textContent = hidden ? full : new Array(25).join("•");
				text.setAttribute("data-revealed", hidden ? "1" : "0");
			}
		});
	...
	function boot() {
		injectStyles();
		// Prefer the plugin's own mount point; fall back to the preview host
		// (#root) or create one so the widget always renders.
